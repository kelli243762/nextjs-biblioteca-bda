...
export default async function ActividadSociosPage({
  ...
  const total = await query(
    `SELECT COUNT(*),
     ...
     ROUND(AVG(tasa_atraso), 2) as promedio_atraso
     FROM vw_member_activity ${whereClause}`,
    queryParams
  );
  ...
  const nivelColor: Record<string, string> = {
    ...
    'poco activo':   'bg-yellow-100 text-yellow-700',
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
            <p className="text-sm text-blue-600">Total préstamos</p>
              ...
              {result.rows.map((row, i) => (
                <tr key={i} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${nivelColor[row.nivel_actividad] || 'bg-gray-100 text-gray-500'}`}>
                      {row.nivel_actividad}
                    </span>
                  </td>
                </tr>
        ...
        <div className="flex gap-2 mt-6 justify-center">
          {page > 1 && (
              ...
              href={`/reports/actividad-socios?page=${page - 1}&member_type=${member_type}`}
